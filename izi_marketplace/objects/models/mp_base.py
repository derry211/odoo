...
class MarketplaceBase(models.AbstractModel):
    ...
    def create_records(self, raw_data, mp_data, multi=False):
        ...
        if multi:
            ...
            for index, single_mp_data in enumerate(mp_datas):
                message_error = False
                mp_exid = single_mp_data.get('mp_exid', False)
                try:
                    records |= self.create_records(raw_datas[index], single_mp_data)
                    self._logger(marketplace,
                ...
                notes = single_mp_data.get('mp_product_name', False)
                notes = single_mp_data.get('name', False) if not notes else notes
                if message_error:
                    log_values = {
                        'name': message_error,
